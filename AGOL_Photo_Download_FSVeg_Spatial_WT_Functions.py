...
def urlRequest(inputUrl, *urlParameters):
      try:
        ...
        arcpy.AddMessage('Unable to make URL request. Check your internet\
...
def generateAGOLToken(AGOLUsername, AGOLPassword):
    ...
    try:
      ...
      arcpy.AddMessage('There was a problem generating an AGOL token.\
...
class AGOLFeatureServiceRESTEndPoints():
    ...
    in AGOL using Python 2.7 because the FS is still using desktop in Citrix
    '''
    ...
    def layerHasPhotoAttachments(self):
        areThereAttachments = urlRequest('{0}/{1}?&f=json&token={2}'.format(self.url, self.layerNumber, self.token))
    ...
    def name(self):
        ...
        '''
        AGOLFeatureServiceName = urlRequest('{0}?&f=json&token={1}'.format(self.url, self.token))
        AGOLFeatureServiceName = str(json.loads(AGOLFeatureServiceName)['layers'][0]['name'])
    ...
    def getFeatureServiceObjectIds(self):
        ...
        '''
        featureServiceObjectIDs = urlRequest('{0}/{1}/query?where=1=1&returnIdsOnly=true&f=json&token={2}'.format(self.url, self.layerNumber, self.token))
        featureServiceObjectIDs = json.loads(featureServiceObjectIDs)['objectIds'][:]
        featureServiceObjectIDs = ','.join(str(featureServiceObjectID) for featureServiceObjectID in featureServiceObjectIDs)
        return featureServiceObjectIDs

    def getFeatureServiceObjectIdsWithinAreaOfInterest(self, areaOfInterestVerticesDictionary):
        ...
        is a result of delete field collected data after syncing
        '''
        urlRequestString = '{0}/{1}/query?where=1=1&returnIdsOnly=true&f=json&geometryType=esriGeometryPolygon&geometry={2}&token={3}'
        featureServiceObjectIDs = urlRequest(urlRequestString.format(self.url, self.layerNumber, areaOfInterestVerticesDictionary, self.token))
        featureServiceObjectIDs = json.loads(featureServiceObjectIDs)['objectIds'][:]
        featureServiceObjectIDs = ','.join(str(featureServiceObjectID) for featureServiceObjectID in featureServiceObjectIDs)
        return featureServiceObjectIDs
    ...
    def queryFeatureServiceObjectIDsforAttachments(self, AGOLfeatureServiceOjbectIDs):
        ...
        '''
        urlEncodedParameters = urllib.urlencode({'objectIds' : AGOLfeatureServiceOjbectIDs})
        urlRequestString = '{0}/{1}/queryAttachments?&f=json&token={2}'
        AGOLfeatureServiceObjectIDsThatHaveAttachments = urlRequest(urlRequestString.format(self.url, self.layerNumber, self.token), urlEncodedParameters)
        AGOLfeatureServiceObjectIDsThatHaveAttachments = json.loads(AGOLfeatureServiceObjectIDsThatHaveAttachments)['attachmentGroups']
        AGOLfeatureServiceObjectIDsThatHaveAttachments = [objectIDWithPhoto['parentObjectId'] for objectIDWithPhoto in AGOLfeatureServiceObjectIDsThatHaveAttachments]
        AGOLfeatureServiceObjectIDsThatHaveAttachments = ','.join(str(featureServiceObjectIDsThatHaveAttachment) for featureServiceObjectIDsThatHaveAttachment in AGOLfeatureServiceObjectIDsThatHaveAttachments)
        return AGOLfeatureServiceObjectIDsThatHaveAttachments

def DeleteUneededFiedsFromFinalFeatureclass(finalFeatureClass):
    listOfFieldsToKeep = []
    fieldsToDelete = [field.name for field in arcpy.ListFields(finalFeatureClass) if field.name not in listOfFieldsToKeep]
    arcpy.DeleteField_management('finalFeatureClass', fieldsToDelete)

def getStatusURLForFeatureServiceReplicaForPhotoAttachments(AGOLFeatureServiceName, AGOLFeatureServiceURL, AGOLToken, AGOLFeatureServiceLayerNumber, AGOLFeatureServiceObjectIDs):
    replicaParameters = urllib.urlencode({
      "name" : AGOLFeatureServiceName,
      "layers" : AGOLFeatureServiceLayerNumber,
      "layerQueries": {AGOLFeatureServiceLayerNumber:{"where":'OBJECTID IN (' + AGOLFeatureServiceObjectIDs + ')'}},
    ...
    AGOLCreateFeatureServieReplicaURL = '{0}/createReplica/?f=json&token={1}'.format(AGOLFeatureServiceURL, AGOLToken)
    AGOLCreateFeatureServieReplicaURLRequest = urlRequest(AGOLCreateFeatureServieReplicaURL, replicaParameters)
...
def waitForAGOLFeatureServiceReplica(AGOLFeatureServiceReplicaStatusURL, AGOLToken):
    timer = 0
    status = json.loads(urllib.urlopen("{0}?f=json&token={1}".format(AGOLFeatureServiceReplicaStatusURL, AGOLToken)).read())
    while status['resultUrl'] == '':
        time.sleep(10)
        timer += 10
        status = json.loads(urllib.urlopen("{0}?f=json&token={1}".format(AGOLFeatureServiceReplicaStatusURL, AGOLToken)).read())
        if timer > 1000:
            raise Exception ('It took too long to make the AGOL Repica. Try again at a different time')
        if status['status'] in ('Failed', 'CompletedWithErrors'):
            raise Exception ('There was an error creating the AGOL Replica. Check your inputs and try again')
    else:
        AGOLFeatureServiceReplicaResultURL = status['resultUrl']
        return AGOLFeatureServiceReplicaResultURL

def downloadAGOLReplicaInFGDB(AGOLReplicaResultURL, AGOLToken, AGOLFeatureServiceName, outputLocation):
    urlResponse = requests.get("{0}?token={1}".format(AGOLReplicaResultURL, AGOLToken), stream=True)
    nameOfAGOLReplicaZipFile =  unicode('{}.zip'.format(AGOLFeatureServiceName))
    fullPathAGOLReplicaOfZipFile = os.path.join(outputLocation, nameOfAGOLReplicaZipFile)
    with open(fullPathAGOLReplicaOfZipFile , "wb") as AGOLReplicaOfZipFile:
      ...
      for data in urlResponse.iter_content(chunk_size = sizeOfFile/10):
          arcpy.AddMessage(str(int((float(chunk)/float(sizeOfFile))*100)) + "% of the file has downloaded")
...
def projectFeatureClassToGCSWGS_84IntoDefaultWorkspace(featureClass):
    ...
    arcpy.Project_management(featureClass, arcpy.env.workspace + '/projectedAreaofInterest', projection)
...
def unzipAGOLReplicaGDBAndRenameToFSVeg(pathOfZippedReplicaGDB, outputLocation):
    with zipfile.ZipFile(pathOfZippedReplicaGDB, 'r') as zipGDB:
        ...
        arcpy.Rename_management(outputLocation + '/' + uniqueAGOLGenerateReplicaGDBName, 'FSVeg_Spatial_WT')
...
def renamePlotsToFSVeg_Spatial_WT_PhotosInGDB(outputLocation):
    ...
    arcpy.arcpy.FeatureClassToFeatureClass_conversion('plots', outputLocation + '/FSVeg_Spatial_WT.gdb', 'FSVeg_Spatial_WT_Photos')
...
def writeAttachedPhotosAndMakeDictionaryOfFSVegPhotoNames(outputLocation, FSVegGlobalIDDictionary):
    ...
    arcpy.env.workspace = outputLocation + '/FSVeg_Spatial_WT.gdb'
    photoNameDictionary = {}
    with arcpy.da.SearchCursor('FSVeg_Spatial_WT_Photos__ATTACH', ['DATA', 'ATT_NAME', 'REL_GLOBALID']) as cursor:
        for row in cursor:
            if row[2] in FSVegGlobalIDDictionary:
                attachment = row[0]
                fileNumber = str(FSVegGlobalIDDictionary[row[2]][0]) + "_plot" + str(FSVegGlobalIDDictionary[row[2]][1])
                attachmentName = str(row[1])
                stringStartLocation = attachmentName.find('photo_plot')
                if attachmentName[stringStartLocation + 10: stringStartLocation + 11] == '-':
                  filename = fileNumber  + "_1.jpg"
                else:
                  filename = fileNumber  + '_' + attachmentName[stringStartLocation + 11: stringStartLocation + 12] + '.jpg'
...
def addPhotoNameFieldAndPopulateFinalFSVegFeatureClass(outputLocation, photoNameDictionary):
    arcpy.env.workspace = outputLocation + '/FSVeg_Spatial_WT.gdb'
...
def DeleteUneededFiedsFromFinalFSVegFeatureclass(outputLocation):
    arcpy.env.workspace = outputLocation + '/FSVeg_Spatial_WT.gdb'
    listOfFieldsToKeep = ['pl_setting_id', 'plot_number_1', 'photo_1_text', 'PhotoNames']
    fieldsToDelete = [field.name for field in arcpy.ListFields('FSVeg_Spatial_WT_Photos') if field.name not in listOfFieldsToKeep and not field.required]

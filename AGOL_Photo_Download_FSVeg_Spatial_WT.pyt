...
class Toolbox(object):
    def __init__(self):
        ...
        .pyt file)."""
        self.label = "A toolbox to download photos attached to editalbe feature services in ArcGIS Online"
        self.alias = ""
...
class DownloadAGOLPhotos(object):
    ...
    def execute(self, parameters, messages):
        ...
        GroupUrl = AGOLGroupUrl + "/1/query?where=1=1"
        if ProjectArea:
        ...
        try:
          ...
          arcpy.AddMessage('Given the parameters that you entered there are no plots which contain photos. Consider using a different project area or filter date and rerun this toolbox. This program will exit in 10 seconds.')
          time.sleep(10)
          exit()
        ...
        if not thisJob['features']:
          ...
          exit()
            ...
            replicaParameters = urllib.urlencode({
              'name': name,
              'layers':"1",
              "layerQueries": {"1":{"where":'OBJECTID IN (' + objectids + ')'}},
            ...
            thisJob = json.loads(createReplReq)
            arcpy.AddMessage('Downloading Photo Attachments for feature Layer Plot Group 1')
            ...
            outDB =  unicode(name + '_Plot_Group_1.zip')
            ...
            if ProjectArea:
                arcpy.MakeFeatureLayer_management(outputLocation + '/FSVeg_Spatial_WT.gdb/plot_group_1', 'pts')
                arcpy.MakeFeatureLayer_management(arcpy.env.workspace + '/ProprojectArea', "ProprojectArea")
                ...
                arcpy.Delete_management(outputLocation + '/FSVeg_Spatial_WT.gdb/plot_group_1')
                arcpy.Delete_management(outputLocation + '/FSVeg_Spatial_WT.gdb/plot_group_1__ATTACH')
                arcpy.Delete_management(outputLocation + '/FSVeg_Spatial_WT.gdb/plot_group_1__ATTACHREL')
            else:
                arcpy.env.workspace = outputLocation + '/FSVeg_Spatial_WT.gdb'
                arcpy.Rename_management('plot_group_1','FSVeg_Spatial_WT_Photos')
                arcpy.Rename_management('plot_group_1__ATTACH','FSVeg_Spatial_WT_Photos__ATTACH')
                arcpy.Rename_management('plot_group_1__ATTACHREL','FSVeg_Spatial_WT_Photos__ATTACHREL')
            arcpy.env.workspace = outputLocation + '/FSVeg_Spatial_WT.gdb'
            inTable = 'FSVeg_Spatial_WT_Photos__ATTACH'
